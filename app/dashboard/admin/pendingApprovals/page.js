...
import { Download, Upload, PackageCheck, Eye } from "lucide-react";
...
export default function ApprovalPendingPage() {
    ...
    const handleSubmitModal = async () => {
        try {
            ...
            // Send email reply on the same thread
            const emailPayload = {
                message: modalMessage,
                attachments: uploadedFiles,
                reuploadCdr: modalReupload === 'yes' || modalReupload === true,
                action: 'reject' // This will mark the order as not approved
            };

            const emailRes = await axios.post(`/api/email/reply/${modalSaleId}`, emailPayload);
            if (!emailRes.data?.success) {
                throw new Error(emailRes.data?.message || 'Failed to send email reply');
            }

            toast.success('Email reply sent and order marked as not approved');
            setShowModal(false);
    ...
    useEffect(() => {
        ...
        axios
            ...
            .then((res) => {
                if (!mounted) return;
                // show only orders created by admin users
                setItems((res.data?.data || []).filter(s => s.creatorRole === 'admin'));
            })
    ...
    // listen for sales updates and update list (keep admin-created filter)
    useEffect(() => {
        const handler = async (e) => {
            try {
                ...
                if (res.data?.success) {
                    ...
                    // keep admin-created orders with mailsent, notApproved, or reuploadedcdr in the pending list
                    if (updated.creatorRole === 'admin' && (updated.orderStatus === 'mailsent' || updated.orderStatus === 'approvalpending' || updated.orderStatus === 'notApproved' || updated.orderStatus === 'reuploadedcdr')) {
