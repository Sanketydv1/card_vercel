...
export default function OrderListPage() {
    ...
    const handleRepeat = async (id) => {
        try {
            if (!window.confirm("Are you sure you want to repeat order like this?")) return;
            setLoading(true);
            const res = await axios.post(`/api/sales/${id}/repeat`);
            if (res.data?.success) {
                // keep UI showing root orders only; add new copy to orders to update counters
                setOrders((prev) => [...prev, res.data.data]);
                toast.success("Order repeated");
                if (typeof window !== "undefined") {
                    window.dispatchEvent(new CustomEvent("sales-updated", { detail: { id: res.data.data._id } }));
                }
            } else {
                toast.error(res.data?.message || "Failed to repeat order");
            }
        } catch (err) {
            toast.error(err?.message || "Failed to repeat order");
        } finally {
            setLoading(false);
        }
    };
    ...
    useEffect(() => {
        const handler = async (e) => {
            try {
                ...
                if (!id) {
                    const res = await axios.get("/api/sales?status=mailsent");
                    setOrders(res.data?.data || []);
                    return;
                }
                ...
                if (res.data?.success) {
                    ...
                    if (updated.orderStatus === 'mailsent') {
                        setOrders(prev => {
                                    ...
                                    <tr key={o._id} className="border-b hover:bg-gray-50 text-center">
                                        ...
                                        <td className="px-6 py-3">
                                            <div className="flex flex-col items-center gap-2">
                                                <span className="inline-block px-3 py-1 rounded-full text-xs font-semibold bg-green-200 text-green-800">submitted</span>
                                            </div>
                                        </td>
                                    </tr>
                                );
